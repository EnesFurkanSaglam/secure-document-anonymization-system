...
class ReviewerService:
    ...
    def submit_review_service(email, tracking_code, review_text, is_final=False):
        ...
        if not assignment:
            return {"error": "This article has not been assigned to this reviewer or the assignment is not active."}, 400
        new_review = Review(
        ...
        return {
            ...
            "is_final": is_final,
            "review_pdf_path": article.review_pdf_path 
        }, 200
    ...
    def publish_article_service(article_id, reviewer_id=None):
        ...
        with open(article.original_pdf_path, "rb") as f:
            enc_data = f.read()
        dec_data = EncryptionService.decrypt_data(enc_data)
        with open(published_path, "wb") as pub:
            pub.write(dec_data)
